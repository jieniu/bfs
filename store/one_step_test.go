...
func TestOneStep(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		if phase, err = httpPostMultipartAndResp(apiUriUpload, values); err != nil {
	...
	if phase, err = httpPostMultipartAndResp(apiUriUploads, values); err != nil {
	...
	for i := 1; i <= 10; i++ {
		...
		if err = ioutil.WriteFile(fmt.Sprintf(dlFileName, i), buf.Bytes(),
			...
			if srcMd5, phase, err = GetMd5OfFile(fmt.Sprintf(testFileName, i)); err != nil {
				t.Errorf("%s %s error(%v)", "Calculate MD5", phase, err)
				t.FailNow()
			}
			if dlMd5, phase, err = GetMd5OfFile(fmt.Sprintf(dlFileName, i)); err != nil {
				t.Errorf("%s %s error(%v)", "Calculate MD5", phase, err)
				t.FailNow()
			}
			if srcMd5 != dlMd5 {
